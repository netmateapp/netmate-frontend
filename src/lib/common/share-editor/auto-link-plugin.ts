...
function $createAutoLinkNode_(
  ...
  if (nodes.length === 1) {
    ...
    console.log(linkNode.__parent);
    // キャレットが最初のテキストノードにあった場合、キャレットの位置は保持されないので、
    // キャレットの位置を復元する必要がある。
...
// Bad Neighborとは、AutoLinksの互換性を失わせるような隣接ノードの編集のことです。
// 作成前提条件を考えると、これらのノードは単純なテキスト・ノードである可能性があります。
...
export function registerAutoLinkPlugin(editor: LexicalEditor): () => void {
