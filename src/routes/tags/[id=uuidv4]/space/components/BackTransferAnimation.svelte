<script lang="ts">
  ...
  const CLIP_PATH_FROM: number = $derived(61 * (1 / currentSpaceScale.scale));
  let clipPathTo: number = 30.5;
  const BACKGROUND_START_COLOR: string = "ffffff";
  const BACKGROUND_END_COLOR: string = "fcfcfc";
  const SCALE_FROM: number = $derived(2 * (1 / currentSpaceScale.scale));
  let scaleTo: number = 1.0;
